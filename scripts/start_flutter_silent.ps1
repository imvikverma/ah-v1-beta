...
try {
    ...
    try {
        ...
        # Build list of candidate Flutter paths
        $flutterPaths = @()

        # 1) Highest priority: explicit override via environment variable
        if ($env:AURUM_FLUTTER_PATH) {
            $flutterPaths += $env:AURUM_FLUTTER_PATH
            Write-Log "Using AURUM_FLUTTER_PATH override: $($env:AURUM_FLUTTER_PATH)" "INFO"
        }

        # 2) Common per-user Develop path (your previous install location)
        $flutterPaths += "C:\Users\$env:USERNAME\Develop\flutter\bin\flutter.bat"

        # 3) Other common Flutter paths
        $flutterPaths += @(
