...
def get_system_stats():
    ...
    try:
        ...
        test_users = User.query.filter_by(is_test=True).count()
        ...
        total_capital = db.session.query(db.func.sum(User.internal_capital)).scalar() or 0

        # Get trading statistics (mock data for now - would come from trading engine)
        trading_stats = {
            'total_trades': 2847,
            'success_rate': 68.5,
            'total_pnl': 425000.0,
            'avg_trade_return': 149.28
        }
        ...
        return jsonify({
            ...
            'capital_stats': {
                ...
                'avg_capital_per_user': float(total_capital) / max(total_users, 1)
            },
...
def get_admin_reports():
    """
    Get administrative reports and analytics.
    """
    if request.method == 'OPTIONS':
        return '', 200

    try:
        # Revenue analytics (mock data for now - would come from settlement engine)
        revenue_data = {
            'platform_fees': 215000.0,
            'tax_reserves': 347000.0,
            'net_revenue': 182000.0,
            'monthly_growth': 12.5
        }

        # User activity (mock data for now - would come from analytics engine)
        user_activity = {
            'active_users': 127,
            'new_signups': 23,
            'avg_session_time': 2700,  # seconds
            'retention_rate': 78.5
        }

        # Performance metrics (mock data for now - would come from monitoring)
        performance = {
...
@admin_bp.route('/users/<user_id>/status', methods=['POST', 'OPTIONS'])
@require_admin
def toggle_user_status(user_id):
    ...
    try:
        ...
        # Find user by ID or user_code
        user = User.query.filter(
            db.or_(User.id == user_id, User.user_code == user_id)
        ).first()
        ...
        return jsonify({
            ...
            'message': f'User {user.user_code} {"activated" if activate else "deactivated"} successfully'
        }), 200
