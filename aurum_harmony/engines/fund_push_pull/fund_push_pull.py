...
class FundPushPullEngine:
    ...
    def push_funds(
        ...
        """
        Push funds FROM Demat TO specified destination.

        Destinations:
        - "savings": Regular savings account
        - "savings_locked": Locked savings (tax reserve)
        - "zenithpulse_tech_account": ZenithPulse Tech Account (30% platform fee)
        - "bank": External bank account

        Direction: Demat → Destination
        Effect: Decreases demat balance, moves funds to destination account
        """
        """
