...
def get_kotak_client_from_env() -> Optional[object]:
    ...
    """
    try:
        from aurum_harmony.api.kotak_neo import KotakNeoAPI
        ...
        if client.authenticate():
            logger.info("Kotak Neo client created and authenticated from environment")
            return client
        else:
            logger.debug("Kotak Neo client created but authentication failed")
            return None
...
def get_hdfc_client_from_env() -> Optional[object]:
    ...
    """
    try:
        from aurum_harmony.api.hdfc_sky_api import HDFCSkyAPI
        ...
        if client.authenticate():
            ...
            logger.debug("HDFC Sky client created but authentication failed")
            return None
