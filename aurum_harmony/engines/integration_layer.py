...
class TradingSystemIntegration:
    ...
    def process_settlement_with_integration(
        ...
        Enhanced Flow:
        1. Track profit and check increment
        2. Calculate settlement with loss buffer
        3. Handle fund transfers (30% → ZenithPulse, 39% → Savings, net → Savings)
        ...
        Args:
            user_id: User identifier
            gross_profit: Gross profit amount (after broker auto-deductions)
        ...
        try:
            ...
            from aurum_harmony.app.orchestrator import CapitalSettlementOrchestrator

            # Step 1: Track profit and check for capital increment
            profit_tracking = CapitalSettlementOrchestrator.track_profit_and_check_increment(
                user_id=int(user_id),
            ...
            # Get updated capital after potential increment
            updated_capital = profit_tracking["current_capital"]

            # Step 2: Calculate enhanced settlement with loss buffer
            ...
            # Step 3: Handle fund transfers according to enhanced flow
            # 30% Platform Fee → ZenithPulse Tech Account
            platform_fee = settlement_data.get("platform_fee", 0)
            if platform_fee > 0:
                self.fund_engine.push_funds(
                    user_id=user_id,
                    amount=platform_fee,
                    reason="Settlement: 30% Platform Fee → ZenithPulse",
                    destination="zenithpulse_tech_account"
                )

            # 39% Tax Reserve → Savings Account (locked)
            tax_locked = settlement_data.get("tax_locked_savings", 0)
            if tax_locked > 0:
                self.fund_engine.push_funds(
                    user_id=user_id,
                    amount=tax_locked,
                    reason="Settlement: 39% Tax Reserve → Savings (locked)",
                    destination="savings_locked"
                )

            # Net to Savings (after all deductions)
            ...
            # Rounding buffer stays in demat (no transfer needed)
            rounding_buffer = settlement_data.get("rounding_buffer_in_demat", 0)
            if rounding_buffer > 0:
                logger.debug(f"Rounding buffer ₹{rounding_buffer:,.2f} stays in demat for user {user_id}")

            # Handle capital increment (if triggered)
            if profit_tracking["increment_triggered"]:
                next_capital = profit_tracking["next_capital"]
