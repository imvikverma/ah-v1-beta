...
class AuthService:
    ...
    def register_user(
        ...
        # New users always start as "new" and non-test; TierManager will use internal_capital
        if hasattr(User, 'user_type'):
            user_data['user_type'] = 'new'
        if hasattr(User, 'is_test'):
            user_data['is_test'] = False
    ...
    def login_user(email: Optional[str] = None, phone: Optional[str] = None, password: str = "") -> Dict[str, Any]:
        ...
        # Generate session token with user_type and is_test
        user_type = getattr(user, 'user_type', 'new')
        is_test = getattr(user, 'is_test', False)
        token = generate_session_token(user.id, user_type=user_type, is_test=is_test)
    ...
    def get_user_from_token(token: str) -> Optional[User]:
        ...
        # Check if session is expired (with 5-minute grace period for clock skew)
        now = datetime.utcnow()
        grace_period = timedelta(minutes=5)
        
        if session.expires_at and now > (session.expires_at + grace_period):
            # Session expired beyond grace period
            logger.debug(f"Session expired for user {user_id}: expires_at={session.expires_at}, now={now}")
            return None
        
        # Update last accessed
        session.last_accessed = now
        
        # Extend session expiration if it's within 24 hours of expiring (auto-refresh)
        # This keeps active users logged in
        if session.expires_at:
            time_until_expiry = session.expires_at - now
            if time_until_expiry < timedelta(hours=24):
                # Extend session by another 7 days if user is active
                session.expires_at = now + timedelta(hours=168)  # 7 days
                logger.debug(f"Extended session expiration for user {user_id} to {session.expires_at}")
