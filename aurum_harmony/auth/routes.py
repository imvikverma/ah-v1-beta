...
from ..app.orchestrator import CapitalSettlementOrchestrator
...
def register():
    ...
    if result['success']:
        ...
        try:
            # Calculate initial capital for new user
            capital_result = CapitalSettlementOrchestrator.calculate_initial_capital_for_user(
                user_id=user_id,
                base_capital=10000.0,  # Default ₹10K base
                num_indices=3,  # NIFTY50, BANKNIFTY, SENSEX
                num_brokers=1,  # Default 1 broker
                num_users=1,  # Single user for now
                user_type="normal"  # Default to normal user
            )

            # Include capital info in response
            response_data = {
                'message': 'User registered successfully',
                'user': user_dict,
                'capital_calculation': {
                    'total_capital': capital_result['total_capital'],
                    'per_index_capital': capital_result['per_index_capital'],
                    'allocation_matrix': capital_result['allocation_matrix']
                }
            }
            ...
            current_app.logger.error(f"Error calculating initial capital for user {user_id}: {e}")
            # Don't fail registration, just log the error
            return jsonify({
                'message': 'User registered successfully (capital calculation pending)',
                'user': result['user']
            }), 201
    else:
...
def login():
    ...
    try:
        ...
        password = data.get('password', '')
        mode = data.get('mode')  # 'user', 'admin', 'test' or None
        ...
        if result['success']:
            ...
            # Get actual user object to check flags
            if user_id:
                user_obj = User.query.get(user_id)
                if user_obj:
                    # Enforce admin / test specific login modes if requested
                    if mode == 'admin' and not user_obj.is_admin:
                        return jsonify({'error': 'Admin access only. Please use an admin account.'}), 403
                    if mode == 'test' and not getattr(user_obj, 'is_test', False):
                        return jsonify({'error': 'This login is restricted to Test Users.'}), 403
...
def complete_onboarding():
    ...
    Sets user_type to 'existing' for users that are currently marked as 'new'.
    """
    if request.method == 'OPTIONS':
        return '', 200

    try:
        user: User = request.current_user  # set by require_auth
        if hasattr(user, 'user_type') and user.user_type == 'new':
            user.user_type = 'existing'
            db.session.commit()
            # Generate new token with updated user_type
            from .jwt_service import generate_session_token
            ...
            return jsonify({
                'success': True, 
                'user': user.to_dict(),
                'token': new_token  # Return new token with updated user_type
            }), 200
