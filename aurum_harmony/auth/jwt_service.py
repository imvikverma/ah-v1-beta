...
class JWTService:
    ...
    def generate_token(self, user_id: int, user_type: str = "new", is_test: bool = False, expires_in_hours: int = 168) -> str:
        """
        Generate a JWT token for a user.
        
        Args:
            user_id: User ID
            user_type: Type of user (new, existing, admin, test)
            is_test: True if user is a test user
...
def generate_session_token(user_id: int, user_type: str = "new", is_test: bool = False) -> str:
    """Generate a session token for a user."""
    return get_jwt_service().generate_token(user_id, user_type=user_type, is_test=is_test)
